...
impl<'a, const DATA_SIZE: usize, const MAX_HEADS: usize, const REGION_COUNT: usize> RegionHeader<MemIo<DATA_SIZE, MAX_HEADS, REGION_COUNT>> for &'a MemRegionHeader<MAX_HEADS> {
    ...
    fn wal_address(&self) -> <MemIo<DATA_SIZE, MAX_HEADS, REGION_COUNT> as IoBackend>::RegionAddress { self.wal_address }
    fn free_list_head(&self) -> <MemIo<DATA_SIZE, MAX_HEADS, REGION_COUNT> as IoBackend>::RegionAddress { self.free_list_head }
    fn free_list_tail(&self) -> <MemIo<DATA_SIZE, MAX_HEADS, REGION_COUNT> as IoBackend>::RegionAddress { self.free_list_tail }
    fn heads(&self) -> &[<MemIo<DATA_SIZE, MAX_HEADS, REGION_COUNT> as IoBackend>::RegionAddress] { &self.heads }
}
...
> IoBackend for MemIo<DATA_SIZE, MAX_HEADS, REGION_COUNT> {
    ...
    fn write_meta(&mut self, region_size: usize, region_count: usize) -> Result<(), IoError<Self::BackingError, Self::RegionAddress>> {
    ...
    fn get_region_address(&mut self, index: usize) -> Result<Self::RegionAddress, IoError<Self::BackingError, Self::RegionAddress>> {
        Ok(index)
    }

    fn get_meta<'a>(&'a mut self) -> Result<Self::StorageMeta<'a>, IoError<Self::BackingError, Self::RegionAddress>> {
        Ok(&self.meta)
    }

    fn get_region_header<'a>(&'a mut self, index: Self::RegionAddress) -> Result<Self::RegionHeader<'a>, IoError<Self::BackingError, Self::RegionAddress>> {
        self.regions
            .get(index)
    ...
    fn write_region_header<'a>(
        ...
        let heads = Vec::from_slice(addresses)
            .map_err(|_| IoError::InvalidHeads)?;
    ...
    fn get_region_data<'a>(&'a mut self, index: Self::RegionAddress, offset: usize, len: usize) -> Result<&'a [u8], IoError<Self::BackingError, Self::RegionAddress>> {
    ...
    fn write_region_data(&mut self, index: Self::RegionAddress, offset: usize, data: &[u8]) -> Result<(), IoError<Self::BackingError, Self::RegionAddress>> {
    ...
    fn get_region_free_pointer(&mut self, index: Self::RegionAddress) -> Result<Option<Self::RegionAddress>, IoError<Self::BackingError, Self::RegionAddress>> {
        self.regions
            .get(index)
            .ok_or(IoError::InvalidAddress(index))
    ...
    fn write_region_free_pointer(&mut self, index: Self::RegionAddress, pointer: Self::RegionAddress) -> Result<(), IoError<Self::BackingError, Self::RegionAddress>> {
