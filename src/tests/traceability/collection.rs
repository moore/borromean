...
fn requirement_map_durability_and_recovery_only_change_when_the_shared_storage_engine_is_used() {
    let mut flash = MockFlash::<512, 16, 2048>::new(0xff);
    let mut workspace = StorageWorkspace::<512>::new();
    let mut storage = Storage::<_, 512, 16, 8>::format(
        &mut flash,
    ...
    let mut reopened =
        Storage::<_, 512, 16, 8>::open(&mut flash, crate::test_storage_memory()).unwrap();
