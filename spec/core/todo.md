...
- [ ] **D21 — Main-WAL root and retained-boundary model.** Agree the main WAL's
  ...
  selection, and the facts replay must retain. The follow-up patch changes only
  the main-WAL introduction in the system narrative.
  ...
  retention of every listed recovery fact. Run Markdown and diff checks, and
  leave D21 unchecked until that bounded patch has been reviewed.
...
### Post-1.0 materialization wear

- [ ] Investigate bounded retry or relocation for repeatedly interrupted
  materialization so crash loops cannot cause excessive erase cycles on one
  preallocated region while preserving already published logical-region
  references.
