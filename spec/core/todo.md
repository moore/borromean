...
- [ ] **D06 — Append and copy-on-write scope.** Agree which core publications
  ...
  patch changes only that introductory claim.
- [ ] **D07 — Discovery and logical-root wording.** Agree that immutable format
  metadata is a fixed anchor, region headers identify candidates, validated WAL
  state selects the tail, and replay selects logical roots. The follow-up patch
  changes only the introductory discovery explanation.
- [ ] **D08 — Qualified wear claim.** Agree the property FIFO reuse provides for
  equally eligible recyclable regions and the exclusions for pinned,
  permanently reserved, and bootstrap regions. The follow-up patch changes only
...
- [ ] **D10 — Logical write and failed-write semantics.** Agree erased-range or
  ...
  follow-up patch changes only the `write` contract.
- [ ] **D11 — Range-sync and failed-sync semantics.** Preserve range sync as the
  API, then decide zero length, partial overlap with an earlier write, widened
  durability, and allowed effects when `sync` returns an error. The follow-up
  patch changes only the `sync` contract.
- [ ] **D12 — Format metadata, version, and geometry.** Agree immutable metadata
...
- [ ] **D17 — Relational ownership vocabulary.** Agree the general meanings of
  Ready Free, Dirty Free, Transaction Owned, user/internal Collection Owned, and
  retained-but-unfinished work. Record transaction-log classification as
  provisional until D27 and D32 settle retention and finish; then feed that
  result back into the summary. The follow-up patch is limited to definitions
  and a short lifecycle summary.
...
- [ ] Specify the collection implementation contract: every allocation made by
  ...
  committed representation. Core assumes this for opaque collection formats
